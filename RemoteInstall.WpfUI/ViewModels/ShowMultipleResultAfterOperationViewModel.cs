...
{
    public class ShowMultipleResultAfterOperationViewModel : BindableBase, INavigationAware
    {
        ...
        private ReadOnlyObservableCollection<StatusInstallOrEdit> _statusesInstallOrEdits;
        public ReadOnlyObservableCollection<StatusInstallOrEdit> StatusesInstallOrEdits
        {
            get => _statusesInstallOrEdits;
            set => SetProperty(ref _statusesInstallOrEdits, value);
        }
        ...
        private OneCBases _oneCBases;
        public OneCBases OneCBases
        {
            get => _oneCBases;
            set => SetProperty(ref _oneCBases, value);
        }
        ...
        public ShowMultipleResultAfterOperationViewModel(IRegionManager regionManager, IInstallOrEditService installOrEditService)
        {
        ...
        {
            if (navigationContext.Parameters["computerNames"] is ObservableCollection<string> computers)
                PopulateCollection(computers);
        ...
        private void PopulateCollection(ObservableCollection<string> computerNames)
        {
            var collection = new ObservableCollection<StatusInstallOrEdit>();
            foreach (string computerName in computerNames)
            {
                collection.Add(new StatusInstallOrEdit { ComputerName = computerName });
            }
            StatusesInstallOrEdits = new ReadOnlyObservableCollection<StatusInstallOrEdit>(collection);
        }

        private void Install()
        {
            Parallel.ForEach(StatusesInstallOrEdits, async (item) =>
            {
                item.StatusMessage = $"Идет установка {InstallSettings.ProductName} на компьютер {item.ComputerName.ToUpper()}";
                ChangeVisibility(VisibilityState.InAction, item);

                var result = await _installOrEditService.Install(item.ComputerName, InstallSettings);

                if (_msiCodeResultes.TryGetValue(result, out string codeMessage))
                {
                    if (result == "0" || result == "3010")
                    {
                        item.StatusMessage = $"Статус установки {InstallSettings.ProductName} на компьютер {item.ComputerName.ToUpper()}: {codeMessage}";
                        ChangeVisibility(VisibilityState.InComplete, item);
                    }
                    else
                    {
                        item.StatusMessage = $"Статус установки {InstallSettings.ProductName} на компьютер {item.ComputerName.ToUpper()}: Ошибка {result} - {codeMessage}";
                        ChangeVisibility(VisibilityState.InError, item);
                    }
                }
                else
                {
        ...
        private void EditBases()
        {
            Parallel.ForEach(StatusesInstallOrEdits, async (item) =>
            {
                item.StatusMessage = $"Идет настройка {OneCBases.NameForUser} на компьютере {item.ComputerName}";
                ChangeVisibility(VisibilityState.InAction, item);
                var result = await _installOrEditService.EditBase(item.ComputerName, OneCBases);
